      ...
      if (this.childrenField) {
        ...
        if (filterValue) {
          this.store.forEach(function (item) {
            item[_this.childrenField].forEach(function (child) {
              if (child[_this.labelField].indexOf(filterValue) > -1) count++;
            });
          });
        } else {
          this.store.forEach(function (item) {
            count += item[_this.childrenField].length;
          });
        }
        return count;
      } else {
        if (filterValue) {
          var _count = 0;
          this.store.forEach(function (item) {
            if (item[_this.labelField].indexOf(filterValue) > -1) _count++;
          });
          return _count;
        }
    ...
    showList: function showList() {
      ...
      if (this.childrenField) {
        ...
        this.store.forEach(function (item) {
          ...
          _this2.$set(_this2.showMap, item[_this2.keyField], false);
          ...
          item[_this2.childrenField].forEach(function (child) {
            ...
            var flag = false;
            if (_this2.filterValue) {
              var label = child[_this2.labelField];
              flag = label.indexOf(_this2.filterValue) > -1;
              if (flag) {
                _this2.$set(_this2.showMap, child[_this2.keyField], true);
              } else {
                resultCount--;
              }
            } else {
              flag = resultCount <= count;
              _this2.$set(_this2.showMap, child[_this2.keyField], flag);
            }

            if (flag) {
              _this2.$set(_this2.showMap, item[_this2.keyField], true);
            }
            resultCount++;
          });
        ...
        this.store.forEach(function (item, i) {
          ...
          if (!_this2.filterValue || _this2.filterValue && item[_this2.labelField].indexOf(_this2.filterValue) > -1) {
            list.push(item);
          }
        });
      }
  ...
  methods: {
    ...
    onFilter: function onFilter(value) {
      ...
      if (this.remote) {
        if (remoteFilterCatch[value]) {
          ...
          this.remote(value).then(function (store) {
            remoteFilterCatch[value] = store;
            _this3.store = store;
            _this3.noDataText = '暂无数据';
          }).catch(function () {
            _this3.store = [];
            _this3.noDataText = '查询失败';
          });
        }
      }
      this.pageNum = 0;
    ...
    onClear: function onClear() {
      ...
      if (!this.childrenField) {
        ...
        this.store.forEach(function (item) {
          item[_this4.childrenField].forEach(function (child) {
